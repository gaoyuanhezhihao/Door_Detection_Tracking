...
char vanish_point_detection(Mat & image, Mat & cdst)
{
	...
	if (false == is_track_task_full) {
		if (ret_code::Get_Door == Detect_door(cdst, edge_im, vertical_lines, doors_points, vp)) {
...
//	return ret_code::No_Door;
	//  match pairs of lines in its left side whose distance is in valid range.
	size_t next_id = 0;
	for (i = 0; i < merged_door_line_candidate.size(); ++i) {
		if (find_next_vline(merged_door_line_candidate, doors_points, next_id, i, edge_im, cdst, vp) == ret_code::Found_a_door) {
			found_door = true;
		}
	}
	if (found_door){
		return ret_code::Get_Door;
	}
	return ret_code::No_Door;
}
/*
